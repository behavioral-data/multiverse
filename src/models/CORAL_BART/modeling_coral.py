...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False, adj_mat = None):
        """
        Args:
        ...
        x, attn_weights = self.self_attn(
            query=x, key=x, key_padding_mask=encoder_padding_mask, output_attentions=output_attentions, adj_mat = adj_mat
        )
...
class BartEncoder(nn.Module):
    ...
    def forward(
        self, input_ids, attention_mask=None, output_attentions=False, output_hidden_states=False, return_dict=False, adj_mat = None
    ):
        """
        Args:
        ...
        for encoder_layer in self.layers:
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn = encoder_layer(x, attention_mask, output_attentions=output_attentions, adj_mat = adj_mat)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:  # don't attend to padding symbols
            ...
            # =======embed ast to reshaped
            if adj_mat is None:
                reshaped = key_padding_mask.unsqueeze(1).unsqueeze(2)
            else:
                reshaped = adj_mat.unsqueeze(1).repeat(1,4,1,1)==0
            # pdb.set_trace()

            # ========attend to neighbors========
            # reshaped = key_padding_mask.unsqueeze(1).unsqueeze(2).repeat(1,4,tgt_len,1)
            # mat = []
            # attend_sz = 100
            # for i in range(tgt_len):
            #     seq = ([1]*i+[0]*attend_sz+[1]*(src_len-i-attend_sz))[:src_len]
            #     assert len(seq)==src_len, i
            #     mat.append(seq)
            # mat = torch.Tensor(mat)
            # mat = (mat.unsqueeze(0).unsqueeze(1).repeat(bsz, self.num_heads, 1,1)==1).cuda()
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "adj_mat" in kwargs:
            adj_mat = kwargs.pop("adj_mat")
        else:
            adj_mat=None
        if decoder_input_ids is None:
            use_cache = False
...
class MaskedBartForConditionalGeneration(PretrainedBartModel):
    base_model_prefix = "model"
    ...
    Conditional generation example::
        ...
        """
        if "adj_mat" in unused:
            adj_mat = unused.pop("adj_mat")
        else:
            adj_mat=None
