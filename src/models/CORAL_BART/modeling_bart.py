...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        # add a type linear layer
        self.type_linear = nn.Linear(config.d_model, config.n_types)
    ...
    Conditional generation example::
        ...
        type_logits = self.type_linear(outputs[0])
        # pdb.set_trace()

        masked_lm_loss = None
        masked_type_loss = None
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            # TODO(SS): do we need to ignore pad tokens in labels?
            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
        if unused["types"] is not None:
            type_loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
            masked_type_loss = type_loss_fct(type_logits.view(-1, self.config.n_types), unused["types"].view(-1))
            # pdb.set_trace()
            # raise NotImplementedError
        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((masked_lm_loss, masked_type_loss) + output) if masked_lm_loss is not None else output
