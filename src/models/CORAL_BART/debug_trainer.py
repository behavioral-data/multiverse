...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                index_of_eos = (inputs["input_ids"].ne(1).sum(dim=1) - 1).unsqueeze(-1)
                a = inputs["input_ids"].gather(1, index_of_eos).squeeze()
                if 1 in a:
                    continue
                    # pdb.set_trace()
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            outputs = model(**inputs)
            if has_labels:
                lm_loss,type_loss, logits = outputs[:3]
